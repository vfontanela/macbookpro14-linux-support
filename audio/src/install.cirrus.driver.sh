...
# --- BEGIN local cache patch (not upstream, added for mbp-cirrus-audio-dkms) ---
# Ubuntu/Debian kernel ABI bumps (eg 7.0.0-27-generic -> 7.0.0-29-generic)
# share the same $kernel_version ("7.0.0") and therefore need the exact same
# patched sound/hda source tree. Cache it locally so routine kernel updates
# don't need network access or a matching linux-source package at all - only
# a genuinely new upstream kernel_version needs a fresh download.
CACHE_DIR="/var/cache/mbp-cirrus-audio-dkms/hda-src"
CACHE_TARBALL="$CACHE_DIR/hda-${kernel_version}.tar.gz"
SKIP_FETCH=false
if [[ -f $CACHE_TARBALL ]]; then
	echo "mbp-cirrus-audio-dkms: using cached patched HDA source for kernel_version $kernel_version ($CACHE_TARBALL) - skipping download"
	mkdir -p "$hda_dir"
	tar -xzf "$CACHE_TARBALL" -C "$build_dir"
	SKIP_FETCH=true
fi
if [[ $SKIP_FETCH != true ]]; then
# --- END local cache patch (closed further down, before popd) ---
...
# --- BEGIN local cache patch (not upstream) ---
fi
# closes "if [[ $SKIP_FETCH != true ]]; then" opened right after the fedora
# patch-package check above, wrapping the whole download/extract/patch block
mkdir -p "$CACHE_DIR"
if [[ $SKIP_FETCH != true ]]; then
	tar -czf "$CACHE_TARBALL" -C "$build_dir" hda
	echo "mbp-cirrus-audio-dkms: cached patched HDA source for kernel_version $kernel_version at $CACHE_TARBALL"
fi
# --- END local cache patch ---
